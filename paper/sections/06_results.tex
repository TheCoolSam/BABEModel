...
\end{align}
Wilcoxon tests on $\Delta_{\text{with trust}}-\Delta_{\text{alone}}$ use a separate Bonferroni family of size $m=7$ after excluding trust-KPI rows with $\Delta_{\mathrm{alone}}\equiv 0$ (Table~\ref{tab:interaction}). Those excluded rows are descriptive Full-vs-TrustOnly contrasts on trust metrics, not Filter$\times$Trust moderation. Opinion clustering is included in the family (CSV); its interaction is non-significant ($W=168$, adj.\ $p=1$).

\begin{table}[htbp]
\centering
\footnotesize
\caption{Filter $\times$ Trust interaction (seed-paired). Primary family $m=7$ excludes trust-KPI rows with $\Delta_{\mathrm{alone}}=0$ (shown below the rule for transparency; adj.\ $p$ n/a).}
...
Opinion Clustering & $-0.0486$ & $-0.0446$ & $+0.2086$ & $168.0$ & $1.0000$ \\
...
Retention interactions (Active Users, Churn, Frustration, Revenue) remain non-significant after Bonferroni. With $n=30$, we report \emph{no evidence of moderation}, not statistical equivalence of Filter effects with and without trust. Active $\sigma$ likewise shows no Filter$\times$Trust interaction ($W=208$, adj.\ $p=1$).

\subsection{Echo Chambers}
Opinion-weighted clustering uses the local active-triple average (Figure~\ref{fig:clustering}). Baseline mean $0.1316$ ($\text{SD} = 0.0255$, $95\%\ \text{CI} = [0.1228, 0.1407]$); Trust Only $0.1316$ ($W = 226.0$, $p_{\text{adjusted}} = 1$). Filter Only lowers clustering to $0.0830$ ($\text{SD} = 0.0152$, $95\%\ \text{CI} = [0.0776, 0.0884]$, $W = 0.0$, $p_{\text{adjusted}} < 1.2 \times 10^{-7}$, $d = 2.32$). Full Model: $0.0870$ ($\text{SD} = 0.0136$, $95\%\ \text{CI} = [0.0821, 0.0919]$, $W = 0.0$, $p_{\text{adjusted}} < 1.2 \times 10^{-7}$, $d = 2.19$). The filter can reduce local same-pole triangles while active-agent $\sigma$ stays high---another active-set nuance.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.58\textwidth]{figures/fig7_echo_chambers.png}
    \caption{Opinion-weighted clustering (local active-triple average). Filter ON conditions reduce local same-pole triangles relative to Baseline while active-agent polarization remains high. Error ribbons: $95\%$ CIs.}
